...
function refreshCommand(client, message, commandName, isSlashCommand) {
  ...
  readdirSync(basePath).forEach(dir => {
    readdirSync(path.join(basePath, dir)).forEach(file => {
      if (path.parse(file).name.toLowerCase() === commandName.toLowerCase()) {
        try {
      ...
      if (guildCommands) {
          guildCommands.forEach(guildCommand => {
              ...
              if (localCommandDescription && guildCommand.description !== localCommandDescription) {
                  client.guilds.cache.get(guildId)?.commands.delete(guildCommand.id);
                  message.channe.send(`Deleted duplicate slash command with mismatched description: ${guildCommand.name}`);
              }
          });
      }
  } catch (error) {
      console.error('Error deleting duplicate guild slash commands with mismatched descriptions:', error);
  }
}