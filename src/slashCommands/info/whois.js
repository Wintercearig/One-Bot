...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('whois')
    .setDescription('Displays a specified user\'s information.')
...
if (boostDate) {
    ...
    if (diffInMonths > 0 && diffInMonths <= 23) {
        ...
        .setAuthor({
          name: `${user.tag} (${member.user.bot ? 'Bot' : member.id === interaction.guild.ownerId ? 'Server Owner' : member.roles.cache.some(role => role.permissions.has([PermissionsBitField.Flags.Administrator])) ? 'Admin' : 'Member'})`,
        ...
        .addFields(
            ...
            { name: `Highest Role`, value: member.roles.highest.id === interaction.guild.id ? 'None' : member.roles.highest.name, inline: true },
        ...
        if (!member.user.bot) {
          ...
          await interaction.reply({ embeds: [uinfoembed] }).catch(console.error);
  }
};